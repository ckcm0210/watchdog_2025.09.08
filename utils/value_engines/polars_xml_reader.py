...
def read_values_from_xlsx_via_polars_xml(xlsx_path: str) -> Dict[str, Dict[str, Optional[str]]]:
    ...
    try:
        ...
        try:
            ...
            print(f"   [gc] 回收前對象: {counts_before}, 回收後: {counts_after}, 釋放: {collected}")
            print(f"   [gc-說明] 第0代 (新對象): {counts_before[0]}->{counts_after[0]}")
            print(f"   [gc-說明] 第1代 (中年對象): {counts_before[1]}->{counts_after[1]}")
            print(f"   [gc-說明] 第2代 (老對象): {counts_before[2]}->{counts_after[2]}")
        except Exception as e:
            print(f"   [gc] 垃圾回收異常: {e}")
